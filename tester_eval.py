...
from evaluator import (
    filter_ips_with_edgeone,
    filter_ips_with_latency,
...
def main():
    ...
    try:
        for group in groups:
            ...
            edgeone_check = to_bool(group.get("edgeoneCheck", False), False)
            if edgeone_check:
                edgeone_host = str(group.get("edgeoneHost", "")).strip() or domain
                items = filter_ips_with_edgeone(
                    ips=ips,
                    domain=edgeone_host,
                    max_count=to_int(group.get("maxCount", 50), 50),
                    tcp_timeout=to_float(group.get("tcpTimeout", 3.0), 3.0),
                    tcp_threshold=to_int(group.get("tcpThreshold", 500), 500),
